...
# 弓を撃ったことを検知
execute if score @s bow matches 1.. run function pvp:system/bow/shotcheck

# 矢を削除
execute at @s if score @s bow matches 1.. run kill @e[sort=nearest,limit=1,distance=0..10,type=arrow]

# 既存の弾にusing_bow.countタグを追加
execute as @e[distance=0..,tag=ammo,type=armor_stand] if score @s bow.count matches 1.. run tag @s add using_bow.count
...
# 新しい弾に射手のUUIDをセット
scoreboard players operation @e[distance=0..,tag=just_spawned,type=armor_stand] UUID.0 = @s UUID.0
scoreboard players operation @e[distance=0..,tag=just_spawned,type=armor_stand] UUID.1 = @s UUID.1
scoreboard players operation @e[distance=0..,tag=just_spawned,type=armor_stand] UUID.2 = @s UUID.2
scoreboard players operation @e[distance=0..,tag=just_spawned,type=armor_stand] UUID.3 = @s UUID.3
...
tag @e[distance=0..,tag=just_spawned,type=armor_stand] remove just_spawned
