...
# ダメージ
    ...
    execute if score @s bow.count matches 1.. at @s positioned ~0 ~-0.8 ~0 if entity @e[tag=shielding,gamemode=!spectator,type=!minecraft:armor_stand,distance=..1,limit=1,sort=nearest,tag=!using_bow_player] run kill @s
    # モブ
    execute if score @s bow.count matches 1.. at @s positioned ~0 ~-0.8 ~0 if entity @e[type=!minecraft:armor_stand,type=!item,type=!marker,type=!minecraft:player,distance=..1,limit=1,sort=nearest,tag=!using_bow_player] as @a[tag=using_bow_player,limit=1] run scoreboard players add @s BowDamage 40
    execute if score @s bow.count matches 1.. at @s positioned ~0 ~-0.8 ~0 run damage @e[type=!minecraft:armor_stand,type=!minecraft:player,distance=..1,limit=1,sort=nearest,tag=!using_bow_player] 4 pvp:bow_damage by @a[tag=using_bow_player,limit=1]
    execute if score @s bow.count matches 1.. at @s positioned ~0 ~-0.8 ~0 if entity @e[type=!minecraft:armor_stand,type=!minecraft:player,distance=..1,limit=1,sort=nearest,tag=!using_bow_player] at @a[tag=using_bow_player,limit=1] run playsound block.amethyst_cluster.hit master @a[tag=using_bow_player,limit=1] ~ ~ ~ 1 2
    execute if score @s bow.count matches 1.. at @s positioned ~0 ~-0.8 ~0 if entity @e[type=!minecraft:armor_stand,type=!minecraft:player,distance=..1,limit=1,sort=nearest,tag=!using_bow_player] run kill @s
