...
# ダメージ
    execute as @e[type=minecraft:armor_stand,tag=ammo,scores={bow.count=2..}] at @s positioned ~0 ~-0.8 ~0 run damage @e[type=!minecraft:armor_stand,distance=..1,limit=1,sort=nearest] 5 pvp:bow_damage
    execute as @e[type=minecraft:armor_stand,tag=ammo,scores={bow.count=2..}] at @s positioned ~0 ~-0.8 ~0 if entity @e[type=!minecraft:armor_stand,distance=..1,limit=1,sort=nearest] run kill
